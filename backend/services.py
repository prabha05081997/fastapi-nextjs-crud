...
async def _lead_selector(lead_id: int, user: _schemas.User, db: _orm.Session):
    lead = (db.query(_models.Lead).filter(_models.Lead.owner_id ==
            user.id, _models.Lead.id == lead_id).first())

    if lead is None:
        raise _fastapi.HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail="Lead does not exists")
